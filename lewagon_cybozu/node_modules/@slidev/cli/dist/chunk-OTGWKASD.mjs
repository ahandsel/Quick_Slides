...
import fs2, {existsSync as existsSync4} from "fs-extra";
...
${code.slice(injectB)}`;
  ...
  async function generateUserStyles() {
    const imports = [
      `import "${toAtFS(join3(clientRoot, "styles/vars.css"))}"`,
      `import "${toAtFS(join3(clientRoot, "styles/index.css"))}"`,
      `import "${toAtFS(join3(clientRoot, "styles/code.css"))}"`
    ];
    const roots2 = uniq3([
      ...themeRoots,
      userRoot
    ...
    for (const root of roots2) {
      const styles = [
        join3(root, "styles", "index.ts"),
        join3(root, "styles", "index.js"),
        join3(root, "styles", "index.css"),
        join3(root, "styles.css"),
        join3(root, "style.css")
      ];
...
${routes.join(",\n")}
  ...
  async function generateGlobalComponents(layer) {
    const components = roots.flatMap((root) => {
      if (layer === "top") {
        return [
          join3(root, "global.vue"),
          join3(root, "global-top.vue"),
          join3(root, "GlobalTop.vue")
        ];
      } else {
        return [
          join3(root, "global-bottom.vue"),
          join3(root, "GlobalBottom.vue")
        ];
      }
    }).filter((i) => fs2.existsSync(i));
    ...
    load(id) {
      ...
      if (match) {
        ...
        return [
          ...
          `import Type from "${slash3(join4(info.dir, typePath))}?raw"`,
